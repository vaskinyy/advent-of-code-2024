...
def execute_program(execution, do_check=True):
    ...
    while True:
        ...
        # print(execution)
        if len(execution.program) < len(execution.output):
            break

    return execution


def iterate_backwards(execution):
    possible_registry_a = [0]
    for i in range(len(execution.program)):
        current = []

        for item in possible_registry_a:
            for offset in range(8):
                register_a = 8 * item + offset

                execution.register_a = register_a
                execution.register_c = initial_c
                execution.register_b = initial_b
                execution.index = 0
                execution.output = []

                execution = execute_program(execution)

                start_idx = len(execution.program) - i - 1
                if execution.output == execution.program[start_idx:]:
                    current.append(register_a)

        possible_registry_a = current

    return min(possible_registry_a)


print(iterate_backwards(execution))
