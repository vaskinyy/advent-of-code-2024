...
def concat_function(items):
    left, right = items
    return int(f'{int(left)}{int(right)}')


print(len(items_pairs))

for idx, pair in enumerate(items_pairs):
    print(f'Processing item #{idx + 1}')
    (number, items) = pair
    places = len(items) - 1

    partial_sums = [(math.prod, 0, 0), (math.fsum, 0, 0), (concat_function, 0, 0)]
    ...
    while partial_sums:
        ...
        if partial_sum < number:
            ...
            partial_sums.append((concat_function, new_partial_sum, start_ids + 1))
